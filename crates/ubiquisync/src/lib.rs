...
// `schema` (+ the `define_schema!`/`define_table!` macros) lets callers declare
// their tables; `define_table!` is pulled in because `define_schema!` expands to
// it. The macros resolve their own `$crate` paths, so re-exporting the names is
// only for `ubiquisync::` ergonomics.
pub use ubiquisync_tables::{define_schema, define_table, id, op, schema};
