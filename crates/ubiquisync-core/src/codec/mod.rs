...
//!
//! A segment is a magic header plus a sequence of entries. Each entry is one
//! op (encoded through the [`Op`] trait), a delta-encoded timestamp, an
//! optional user id (server mode), and a truncated blake3 integrity check.
//!
...
pub use consts::{FLAG_DEVICE, FLAG_SERVER, MAGIC, TAG_EXPUNGED};
